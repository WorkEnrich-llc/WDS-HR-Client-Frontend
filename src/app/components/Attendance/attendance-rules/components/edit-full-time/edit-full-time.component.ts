        ...
        settings: {
          full_time: {
            lateness: this.latenessEntries.map((entry, index) => ({
              index: index + 1,
              value: entry.value !== null ? formatNumber(entry.value) : 0.0
            })),
            early_leave: this.earlyLeaveRows.map((row, index) => ({
              index: index + 1,
              value: row.deduction !== null ? formatNumber(row.deduction) : 0.0
            })),
            ...
            overtime: {
              flat_rate: {
                ...
                value: this.overtimeType === 'flatRate'
                  ? formatNumber(this.flatRateValue)
                  : 0.0
              },
              custom_hours: {
                status: this.allowOvertime && this.overtimeType === 'customHours',
                value: this.overtimeType === 'customHours'
                  ? this.overtimeEntries.map(entry => ({
                    from_time: entry.from || '0:0',
                    to_time: entry.to || '0:0',
                    rate: entry.rate !== null ? formatNumber(entry.rate) : 0.0
                  }))
                  : []
              }
            },
            grace_period: {
              ...
              minutes: this.graceMinutes !== null ? formatNumber(this.graceMinutes) : 0.0
            }
          },
          part_time: {
            lateness: [],
